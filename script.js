...
window.onload = function() {
    ...
    if(investForm) investForm.onsubmit = (e) => {
        ...
        const realized = document.querySelector('input[name="inpRealized"]:checked').value;
    ...
    if(planForm) planForm.onsubmit = (e) => {
        e.preventDefault();
        const realized = document.querySelector('input[name="planRealized"]:checked').value;
        const newP = { 
            id: Date.now(), 
    ...
    if(personalTxForm) personalTxForm.onsubmit = (e) => {
        ...
        const realized = document.querySelector('input[name="ptxRealized"]:checked').value;
        const account = getVal('ptxAccount');
        const toGiro = getVal('ptxToGiro') === 'yes';
        ...
        // Se for despesa E marcou "P/ Giro", cria um depósito automático no Giro
        if(type === 'withdraw' && toGiro) {
            const autoT = {
                dataType: 'transaction',
                id: Date.now() + 0.5,
                date: date,
    ...
    if(giroTxForm) giroTxForm.onsubmit = (e) => {
        ...
        const realized = document.querySelector('input[name="gtxRealized"]:checked').value;
...
function updatePersonalGiroVisibility() {
    const tipo = getVal('ptxType');
    const giroGroup = document.getElementById('personalGiroGroup');
    if(giroGroup) {
        giroGroup.style.display = tipo === 'withdraw' ? 'block' : 'none';
    }
}
...
function updateDashboard() {
    ...
    transactions.forEach(t => {
        ...
        // Excluir transações auto-geradas (sincronização entre carteiras)
        if(t.autoGenerated) return;
